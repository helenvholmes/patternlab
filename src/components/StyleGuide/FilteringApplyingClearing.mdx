...
Filters should be applied to using one of two methods: live filtering or batch filtering.
...
- remove filters (tags) from `active filters` list (`TagSet` component)
- update the `results` list to reflect the `active filters`
